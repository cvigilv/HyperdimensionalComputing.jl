...
RealHV(distr::Distribution, n::Integer=10_000) = RealHV(rand(distr,n))
...
struct GradedBipolarHV{T<:Real} <: AbstractHV{T}
    v::Vector{T}
    GradedBipolarHV(v::AbstractVector{T}) where {T<:Real} = new{T}(clamp!(v,0,1))
end
